...
# Build 37 is applied after Build 36. It preserves the proven graph layout and widget kinds,
# so existing watch-face complication selections remain intact. Only the circular timing
# presentation and exact age thresholds are changed.
...
# Build 36's timeline already creates explicit visual transition entries. Move them from
# 6/12 minutes to the requested full-minute boundaries 7/13 minutes.
...
replacements = {
    "private let dimAfter: TimeInterval = 6 * 60": "private let dimAfter: TimeInterval = 7 * 60",
    "private let staleAfter: TimeInterval = 12 * 60": "private let staleAfter: TimeInterval = 13 * 60",
    "exact local visual transitions at 6 and 12 minutes": "exact local visual transitions at 7 and 13 minutes",
}
for old, new in replacements.items():
    count = provider_text.count(old)
    if count != 1:
        raise RuntimeError(f"Build37 provider patch expected one occurrence of {old!r}, found {count}")
    provider_text = provider_text.replace(old, new, 1)
provider.write_text(provider_text, encoding="utf-8")

print("Build 37 applied: Apple-style 60 minute ticks, CCW age display, white/yellow/red age states at 0-6/7-12/13+ minutes.")
